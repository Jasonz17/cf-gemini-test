...
export class AIService {
  ...
  async generateResponse(
    ...
    const contents = await this.prepareContents(input);
    ...
    if (options.stream) {
      return this.genAI.models.generateContentStream(config);
    }
    return this.genAI.models.generateContent(config);
  }

  private async prepareContents(input: string | File[]) {
    const contents = [];
...
const aiService = new AIService(new GoogleGenAI({ apiKey: '' }));
